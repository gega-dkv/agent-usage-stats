...
export function LineChart({ data, height = 200, color = 'hsl(217, 91%, 60%)', gradient = true }: LineChartProps) {
  if (data.length === 0) {
    return (
      <div
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ height }}>
      ...
      {gradient && <path d={areaPath} fill="url(#lineGrad)" />}
      <path d={linePath} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />

      {points.map((p, i) => (
        <g key={i} className="group">
          <circle cx={p.x} cy={p.y} r="3" fill={color} className="transition-all" />
